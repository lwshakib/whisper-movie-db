  ...
  if (!person) {
    return (
      <View className="flex-1 items-center justify-center bg-white dark:bg-black">
        <Text className="text-lg text-gray-700 dark:text-gray-200">Person not found.</Text>
      </View>
    );
  }
    ...
    <View className="flex-1" style={{ backgroundColor: isDark ? '#000000' : '#ffffff' }}>
      ...
      >
        ...
        <Animated.View entering={FadeInDown.duration(400).springify()} className="-mt-12 mb-6 px-4">
          ...
          <Text className="mb-4 text-center text-sm font-medium uppercase tracking-widest text-gray-500 dark:text-gray-400">
            {person.known_for_department}
          </Text>

          <Text className="mt-2 text-center text-base font-normal leading-6 text-gray-600 dark:text-neutral-300">
        ...
        <View className="mb-4 px-4">
          ...
          <View className="flex-row flex-wrap justify-between">
            <PersonInfoItem title="Gender" value={genderMap[person.gender] || 'Unknown'} />
            <PersonInfoItem title="Birthday" value={person.birthday} />
            <PersonInfoItem title="Place of Birth" value={person.place_of_birth} />
            <PersonInfoItem title="Popularity" value={person.popularity?.toFixed(0)} />
            {person.deathday && <PersonInfoItem title="Deathday" value={person.deathday} />}
            {person.also_known_as?.length > 0 && (
              <PersonInfoItem title="Also Known As" value={person.also_known_as[0]} />
            )}
          </View>
        </View>
              ...
              {movies.map((m: any, index: number) => (
                ...
                >
                  <Image
                    ...
                    className="mb-2 h-56 w-36 rounded-xl bg-gray-200 dark:bg-gray-800"
                    resizeMode="cover"
                  />
                  <Text
